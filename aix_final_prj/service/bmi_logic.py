...
def preprocess_data():
    ...
    X = df1[['Age', 'Gender','Country', 'Sleep_Hours', 'Heart_Rate', 'Physical_Activity_Hours', 'Smoking', 'Occupation', 'Alcohol_Consumption']]
    y = df1['BMI']
...
def run_clustering_analysis():
    ...
    sns.scatterplot(x="PCA1", y="PCA2", hue="Cluster", data=df1, palette="viridis", alpha=0.8)
...
def run_clustering_analysis_compact():
    ...
    X = df1[['BMI','Country', 'Occupation', 'Alcohol_Consumption']]
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    bins = [-np.inf, 18.5, 25, 30, np.inf]
    labels = [0, 1, 2, 3]
    ...
    cluster_analysis_results = []
    clustering_features = ['BMI','Country', 'Occupation', 'Alcohol_Consumption']
...
def get_trained_model():
    if not hasattr(get_trained_model, "model"):
        print("BMI관련 딥러닝 모델을 로드하고 학습합니다...")
        ...
        data = data.drop(['BMI', 'Stress_Level'], axis=1)
        ...
        X_dl = data.drop('BMI_category', axis=1).values
        y_dl = data['BMI_category'].values
...
def predict_bmi_category(input_data):
    ...
    bmi_categories = ['저체중(Underweight)', '정상(Normal)', '과체중(Overweight)', '비만(Obese)']
    predicted_category = bmi_categories[predicted_class_index]
    
    return {
        "predicted_class": predicted_category,
        "predicted_index": predicted_class_index,
        "probabilities": {bmi_categories[i]: f"{p:.4f}" for i, p in enumerate(probabilities)}
    }
